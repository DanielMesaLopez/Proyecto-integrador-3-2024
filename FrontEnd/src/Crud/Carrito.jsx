...
const Carrito = () => {
  ...
  //metodo post
  const add = () => {
    Axios.post("http://localhost:3300/Insertar/carrito", {
  ...
  //metodo get
  const getCarrito = () => {
    Axios.get("http://localhost:3300/Consultar/carrito").then((response) => {
      setCarrito(response.data);
      // alert("Carrito en listados");
    });
  };
  getCarrito();
  ...
  const update = () => {
    Axios.put("http://localhost:3300/Actualizar/carrito", {
      id_carrito: id_carrito,
      id_usuario: id_usuario,
      fecha_compra: fecha_compra,
      fecha_pago: fecha_pago,
  ...
  const deleteCarrito = (val) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        Axios.delete(`http://localhost:3300/Delete/carrito/${val.id_carrito}`)
          .then(() => {
            getCarrito();
            limpiar();
            Swal.fire({
